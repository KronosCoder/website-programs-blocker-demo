...
title Game Blocker - Blocking Mode v1.2.7
color 0C
echo.
echo ========================================
...
echo. >> "%HOSTS%"
echo # ====== GAME BLOCKER v1.2.7 - START ====== >> "%HOSTS%"
...
echo # ====== GAME BLOCKER v1.2.7 - END ====== >> "%HOSTS%"

echo [OK] Gaming websites blocked in HOSTS!
echo.

:: ===================================
:: DISABLE SECURE DNS (DoH) IN BROWSERS
:: ===================================
echo [STEP 1.5] Enforcing Browser Policies (Disable DoH)...

:: Firefox Policy (Disable DNS over HTTPS)
reg add "HKLM\SOFTWARE\Policies\Mozilla\Firefox" /v DNSOverHTTPS /t REG_DWORD /d 0 /f >nul 2>&1

:: Brave Policy (Disable DNS over HTTPS)
reg add "HKLM\SOFTWARE\Policies\BraveSoftware\Brave" /v DnsOverHttpsMode /t REG_SZ /d "off" /f >nul 2>&1

:: Chrome Policy (Disable DNS over HTTPS - Just in case)
...
echo IMPORTANT: You may need to restart your browser completely.
echo.
